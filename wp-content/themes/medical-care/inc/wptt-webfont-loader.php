...
if ( ! class_exists( 'Medical_Care_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Medical_Care_WPTT_WebFont_Loader {
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $medical_care_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $medical_care_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$medical_care_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$medical_care_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$medical_care_font_family = trim( str_replace( array( "'", ';' ), '', $medical_care_font_family ) );
					$medical_care_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $medical_care_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $medical_care_font_family ] ) ) {
					$result[ $medical_care_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$medical_care_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$medical_care_font_family_url = $this->get_absolute_path( $medical_care_font_family_url );

					$result[ $medical_care_font_family ][] = $medical_care_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $medical_care_font_family ] = array_flip( array_flip( $result[ $medical_care_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Medical_Care_WPTT_WebFont_Loader( $url );
